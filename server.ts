...
function editTime(data) {
    ...
    if (
        ...
        (classElement.startsWith("extensible") && (status === "started" || (status === "paused" && enablePauseAdd) || (status === "stopped" && enableStopAdd)))
    ) {
...
app.get("/:classElement/control&:request", (req, res) => {
    ...
    } else if (!classElement.startsWith("time") && request.split("=")[0] === "addtime") {
        if (classElement.startsWith("cdowntime")) {
            editTimeCdowntime(wss, GlobalVariables, `+${request.split("=")[1]}`, classElement);
        } else {
            editTimeTimer(wss, GlobalVariables, `+${request.split("=")[1]}`, classElement);
        }
    } else if (!classElement.startsWith("time") && request.split("=")[0] === "subtime") {
        if (classElement.startsWith("cdowntime")) {
            editTimeCdowntime(wss, GlobalVariables, `-${request.split("=")[1]}`, classElement);
        } else {
            editTimeTimer(wss, GlobalVariables, `-${request.split("=")[1]}`, classElement);
